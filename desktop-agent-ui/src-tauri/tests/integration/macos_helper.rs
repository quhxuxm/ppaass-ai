...
use common::tun_control::{TunHelperRequest, TunHelperResponse, TUN_HELPER_PROTOCOL_VERSION};
use desktop_agent_ui::macos_helper::*;
use std::fs;
...
fn helper_state_paths_are_confined_to_the_socket_directory() {
    let directory = tempfile::tempdir().unwrap();
    let config_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../config/agent.toml");
