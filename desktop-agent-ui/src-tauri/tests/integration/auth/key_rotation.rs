...
use desktop_agent_ui::auth::{
    authenticate_and_download, authenticate_rotate_and_download, validate_device_token_payload,
};
...
fn device_token_accepts_an_admin_with_an_active_profile() {
    ...
    .to_string();
    let downloaded =
        validate_device_token_payload(&payload, "https://proxy.example.com".to_string()).unwrap();
