...
use super::super::{apply_permission_snapshot, fetch_agent_permission_snapshot};
...
async fn permission_sync_uses_bearer_auth_and_accepts_a_rolling_token() {
    let body = serde_json::json!({
        ...
        "profile": {
            "username": "alice",
            "permissions": ["agent.packet_capture", "agent.config.view"],
            "enabled": true,
            "key_version": 7,
    ...
    assert_eq!(snapshot.token.value.as_str(), "B".repeat(43));
    ...
    assert_eq!(
        updated.permissions,
        ["agent.packet_capture", "agent.config.view"]
    );
