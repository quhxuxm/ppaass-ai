...
- `users.toml` is no longer a supported user source or compatibility path. Proxy Registry exclusively owns authoritative account, profile, encrypted-key, Proxy-address, approval, audit and access data. Proxy Entry requests public authorization snapshots through the authenticated control API, never opens Registry databases, and stores only its own public last-known-good replica in local SQLite.
- Proxy Registry must expose public authorizations through revision-bound username keyset pagination, never offset pagination. Every page is limited to 256 entries. Entry writes one validated page at a time to a local SQLite staging table and atomically switches the active replica only after every page for the same revision succeeds; a revision conflict restarts pagination from the first page. Authentication queries the active database by username instead of retaining every authorization in memory. Entry fails closed before the first committed snapshot, retains the last-known-good snapshot indefinitely across Registry outages and Entry restarts, and automatically refreshes after registration or authorization SSE changes. A failed or partial refresh must never clear the active snapshot. Account disablement, deletion, permission changes and key rotation performed during an outage take effect after the next successful full sync.
...
- Creating/editing a user or approving any initial/regeneration key request assigns one to 32 distinct enabled catalog entries using checkboxes. Approval atomically replaces the request owner's assignments. An address that is still assigned cannot be disabled, but administrators may delete any catalog entry; deletion atomically removes its account assignments, leaving accounts with no remaining address in the unassigned fail-closed state.
...
- Desktop TUN DNS proxying must never rewrite operating-system DNS configuration. This prohibition covers physical adapters, macOS network services, and Agent-created TUN/Wintun adapters on both Windows and macOS, including startup, normal operation, shutdown, cleanup, migration and crash recovery. The Agent must capture requests addressed to the DNS servers already selected by the operating system and route the DNS traffic through the in-process `DnsProxy`; it must not invoke `Set-DnsClientServerAddress`, `networksetup -setdnsservers`, or an equivalent write API. New releases must not create DNS-configuration leases. If an older release left a DNS state file, the Agent may warn and preserve it for manual inspection but must not automatically apply its values or overwrite configuration the user may have changed afterward.
- On macOS, TUN route and DNS capture changes must be bound to the selected physical gateway/interface, preserve bypass routes for Registry and assigned Proxy endpoints, and restore system routes/PF state on shutdown. Helper replacement must be version-safe and must not damage an already working installed helper during ordinary UI testing.
