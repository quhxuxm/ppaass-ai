...
- TCP relay 和 UDP relay 的 Yamux 外层连接池在 Agent 侧分开维护，Proxy accept 侧不按入口 TCP 连接预判类型。
...
9. `desktop-agent-be/src/connection_pool/*`：看 TCP/UDP raw Yamux session 池。
