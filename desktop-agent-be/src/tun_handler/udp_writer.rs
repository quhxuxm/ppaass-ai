...
/// Keeps concurrent direct UDP flows off a global async mutex while preserving
/// the netstack sink's required single-writer ordering.
#[derive(Clone)]
...
async fn run_udp_writer(
    ...
    loop {
        ...
        if let Err(error) = sink.feed(first).await {
            debug!(%error, "UDP netstack writer stopped while feeding a packet");
            break;
        }
        ...
        for _ in 1..UDP_WRITE_BATCH_LIMIT {
            ...
            if let Err(error) = sink.feed(packet).await {
                debug!(%error, "UDP netstack writer stopped while feeding a batch");
                return;
            }
        }

        if let Err(error) = sink.flush().await {
            debug!(%error, "UDP netstack writer stopped while flushing");
            break;
        }
    }
}
