use desktop_agent_be::tcp_relay::{TcpRelayOptions, relay_tcp_bidirectional};
...
async fn copy_relay_does_not_cancel_response_when_request_shutdown_errors() {
    ...
    let relay = tokio::spawn(async move {
        relay_tcp_bidirectional(&mut client_relay, &mut remote, TcpRelayOptions::tun("test")).await
    });
