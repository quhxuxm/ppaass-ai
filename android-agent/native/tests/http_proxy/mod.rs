...
use android_agent::http_proxy::{detect_proxy_protocol, request_log_host, run_android_http_proxy};
use android_agent::packet_capture::ProxyIngressProtocol;
use android_agent::{
    AndroidAgentConfig, AndroidTunConfig, DirectAccessConfig, DirectAccessMode,
    http_proxy_clients_json,
};
...
async fn socks5_proxy_connects_to_direct_tcp_target() {
    ...
    let clients = http_proxy_clients_json();
    assert!(
