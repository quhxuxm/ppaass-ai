...
健康检查校验；权限、摘要或有效期不一致都会使登录态失效。应用同时禁止云备份与设备迁移
...
登录页可以选择记住用户名和密码，并提供新用户注册链接；也可以选择“使用浏览器登录”。
浏览器登录会创建一次性的设备授权，在系统浏览器中使用用户名和密码完成登录与设备批准，Agent
只按服务端返回的间隔轮询结果，不在界面展示授权 URL、设备码或私钥。用户可以随时取消，
Activity 销毁或 App 进程结束也会中止轮询。领取凭据后，Agent 会先校验同一响应中的公私钥
配对，再用 CSRF token 和会话 Cookie 尽力注销临时 Web 会话。

记住的登录信息按产品要求存放在应用私有 SharedPreferences 中；取消“记住”会立即删除已
保存密码。浏览器设备登录成功会清除旧的记住密码，退出账号会删除托管私钥，但普通密码登录时
仍保留用户选择记住的登录信息。
每次 App 进程重新启动仍会先显示登录页，并删除上个进程遗留的托管私钥，防止后台服务
绕过本次登录。运行过程中一旦密钥到期或托管文件损坏，VPN 与 HTTP/SOCKS5 服务会在健康
检查中停止，界面也会退回登录页；设备级 Mock GEO 也会同步停止并清理。管理员账号不能
用于 Agent 登录。
...
Agent 登录态只存在于当前 App 进程。设备重启、系统回收进程或 App 被强制停止后，Android
可能再次拉起 Always-on Service，但 Service 会保持 fail-closed，直到用户重新打开 App 并
完成登录；它不会静默使用旧私钥恢复 VPN。若同时启用了 Android 的“阻止无 VPN 连接”，
该系统策略可能阻止登录请求访问 Proxy Web，用户需要先临时关闭 lockdown、完成登录并
启动 VPN，再按需要重新启用。
