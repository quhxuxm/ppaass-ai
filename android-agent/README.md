...
- 应用层 UDP/443 QUIC 命中 direct 规则时使用受保护 UDP socket 直连；未命中时，全 TCP 模式通过 proxy UDP relay，混合模式会阻断并让应用回退 TCP/TLS，避免 HTTP/3 套在可靠 QUIC 流上产生队头阻塞。需要强制所有应用 QUIC 回退时可选择“阻断 UDP/443”。
...
TUN 地址和 MTU 是 Android App 内部固定配置，分别为 `10.10.10.2/24`、禁用 IPv6、MTU 1500，因此 UI 中不会展示这些选项。Android 会指向 VPN 网络路径内的一个 routed DNS 地址；Rust 会根据 `direct_access` 域名规则决定 DNS 查询直连还是映射为 `ProxyDns`。UDP/443 QUIC 命中 direct 规则时继续直连；未命中时，全 TCP 模式通过 proxy UDP relay，混合模式会让应用回退 TCP/TLS。需要强制所有应用 QUIC 回退时可选择“阻断 UDP/443”。
