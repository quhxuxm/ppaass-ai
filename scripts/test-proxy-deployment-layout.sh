...
require_text .github/workflows/deploy-proxy-entry.yml "secrets[format('{0}_AUTH_CONNECT_PRIVATE_KEY', inputs.environment)]"
require_text .github/workflows/deploy-proxy-entry.yml 'auth-connect-private-key.pem'
...
require_text deploy/proxy-entry/install.sh 'Missing AuthConnect private key in deployment bundle.'
require_text deploy/proxy-entry/install.sh 'auth-connect-private-key.pem" "$secret_root/auth-connect-private-key.pem"'
...
require_text deploy/proxy-registry/install.sh 'retire_legacy_caddy_service'
require_text deploy/proxy-registry/install.sh 'systemctl disable "$legacy_service"'
require_text deploy/proxy-registry/install.sh 'systemctl stop "$legacy_service"'
require_text deploy/proxy-registry/install.sh 'rm -f -- "$legacy_unit"'
require_text deploy/proxy-registry/install.sh 'rm -f -- /etc/ppaass/Caddyfile'
require_text deploy/proxy-registry/install.sh 'rm -rf -- /var/lib/ppaass-caddy'
...
require_text deploy/proxy-registry/install.sh 'issuer acme https://acme-v02.api.letsencrypt.org/directory'
require_text deploy/proxy-registry/install.sh 'profile shortlived'
require_text deploy/proxy-registry/install.sh 'disable_http_challenge'
