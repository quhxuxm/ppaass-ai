...
#[tokio::test]
async fn handoff_path_is_the_exact_account_management_endpoint() {
    ...
    let code = handoff_path
        .strip_prefix(HANDOFF_PREFIX)
        .expect("handoff path must use the account-management endpoint");
    assert_eq!(code.len(), 43);
    assert!(!code.contains(['?', '&', '#']));
    assert_eq!(
        handoff_path,
        format!("/api/v1/auth/agent-handoff?code={code}")
    );
}
