...
#[tokio::test]
async fn nickname_is_trimmed_and_limited_by_unicode_characters() {
    let (_directory, app) = test_app().await;
    let (cookie, csrf) = register_user(&app, "nickname-boundary", "profile-password").await;

    let accepted = update_profile(
        &app,
        &cookie,
        &csrf,
        json!({"display_name":"  六个中文字  "}),
    )
    .await;
    assert_eq!(accepted.status(), StatusCode::OK);
    assert_eq!(json_body(accepted).await["display_name"], "六个中文字");

    let rejected = update_profile(
        &app,
        &cookie,
        &csrf,
...
#[tokio::test]
async fn avatar_dimensions_must_be_exactly_64_by_64() {
    let (_directory, app) = test_app().await;
    let (cookie, csrf) = register_user(&app, "avatar-dimensions", "profile-password").await;

    let accepted = update_profile(
        &app,
        &cookie,
        &csrf,
        json!({"avatar_data_url":png_data_url(64, 64)}),
    )
    .await;
    assert_eq!(accepted.status(), StatusCode::OK);
    ...
    let rejected = update_profile(
        ...
        &cookie,
        &csrf,
        json!({"avatar_data_url":png_data_url(64, 63)}),
    )
    .await;
    assert_eq!(rejected.status(), StatusCode::BAD_REQUEST);
}
