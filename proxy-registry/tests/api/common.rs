...
pub(super) use protocol::RsaKeyPair;
pub(super) use proxy_registry::{
...
pub(super) async fn test_app_with_components() -> (
    ...
    let agent_events = AgentEventHub::start(store.clone()).await.unwrap();
    let state = AppState {
    ...
    (
        ...
        private_keys,
        build_router_with_timeout(state, None, Duration::from_secs(180)),
    )
}
