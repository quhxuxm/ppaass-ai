...
pub trait AccountRepository: Send + Sync {
    ...
    /// 持久化用户自选的多个 Proxy Entry。存储层在同一事务中复核权限与分配关系，
...
/// Proxy Entry 远程上报访问记录时使用的批次幂等接口。
///
/// 批次登记与访问次数累加必须在同一事务中提交。调用方重试同一
/// `(entry_id, batch_id)` 时返回 `false`，且不得再次累加访问次数。
#[async_trait]
pub trait AccessBatchRepository: Send + Sync {
