...
impl SqliteUserRepository {
    ...
    ///
    /// Opens a Registry-owned database for read-only maintenance or verification. It neither
    /// creates files nor runs migrations/imports, and every connection has both
    /// `SQLITE_OPEN_READONLY` and `PRAGMA query_only=ON`.
    ...
    pub async fn connect_read_only(path: impl AsRef<Path>) -> Result<Self> {
        ...
        if schema_version != SQLITE_SCHEMA_VERSION {
            return Err(UserRepositoryError::InvalidSchema(format!(
                "Registry 只读用户数据库版本必须为 {SQLITE_SCHEMA_VERSION}，实际为 \
        ...
        info!(
            ...
            "Registry SQLite 用户数据库已以强制只读模式打开"
        );
        Ok(store)
    }
    ...
    async fn migrate(&self) -> Result<()> {
        ...
        if schema_version < 14 {
            create_v14_proxy_entry_selections(&mut transaction).await?;
        }
        if schema_version < 15 {
            migrate_v15_proxy_entry_selections(&mut transaction).await?;
        }
